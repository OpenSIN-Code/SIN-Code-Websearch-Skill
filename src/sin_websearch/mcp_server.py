...
This module lives inside the package so the @mcp.tool() functions can be
imported by both:
  - the root `mcp_server.py` (which serves as the existing entry point), and
  - the `cli_shims/` wrappers (one CLI binary per tool).
"""
